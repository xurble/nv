...
The remaining Phase 2 work is format-preserving rich editing. The codec retains unchanged externally produced RTF/HTML bytes and refuses a text-only mutation of loaded rich content; store tests prove the original file and in-memory note remain unchanged on refusal. A format-preserving model, editor adapter, serializer, and insertion/deletion/formatting fixtures are still required before rich bodies become editable and the complete Phase 2 exit criterion is met.
...
1. **Completed August 2026:** Restored the Phase 1 safety net to green. Clean composed runs now report 840 Debug warnings and 841 Release warnings against the ceiling of 875, retain the corrected `master` push trigger, and reach the app smoke test, both XCTest targets, and every characterization executable.
