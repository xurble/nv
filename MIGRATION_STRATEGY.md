...
The project contains approximately 40,000 lines of Objective-C and C. Its largest and most interconnected classes include `NoteObject`, `AppController`, `NotationController`, `LinkingEditor`, and `NotesTableView`. Phase 1 provides separate XCTest unit and disposable-directory integration targets plus focused characterization executables for critical legacy seams, but the current Debug and Release builds exceed the recorded warning ceiling by two warnings. Phase 2 has extracted a substantial shared storage boundary, and Phase 3 has added a universal mobile target and shared SwiftUI feature, but neither phase has met its stated exit criterion. Representative end-to-end Notational Velocity and nvAlt archives, the production compatibility-to-shared-model bridge, format-preserving RTF/HTML editing, and executed cross-platform UI tests remain required work.
...
- a shared RTF/HTML model that protects existing rich files from destructive plain-string edits but does not yet support format-preserving rich-content editing;
...
The implemented vertical slice shows disposable fixture collections in a SwiftUI `NavigationSplitView` on iPad and Mac and a compact navigation stack on iPhone. It supports search, open, create, plain-text edit, save, rename, tag, pin, delete, and conflict display through `NoteStore`. The Mac shell can host its existing editor while iOS uses a `UITextView` adapter; loaded RTF and HTML bodies remain read-only until format-preserving editing is implemented.
...
**Status as of August 2026 — infrastructure implemented, currently failing:** The shared `Notation` scheme contains distinct `NotationSettingsTests` unit and `NotationIntegrationTests` disposable-directory targets in Debug and Release. Golden fixtures and characterization tests protect TXT, RTF, HTML, every legacy encoding-menu entry, filename sanitization and collision handling, labels, extended attributes, search matching/order/highlighting, import/export round trips, historical synchronization metadata without service activation or fixture rewriting, the legacy encrypted-database KDF/compression/cipher envelope, known cipher vectors, and the production WAL's replay/torn-write/wrong-key behavior. Simulated iCloud policy tests distinguish unavailable and delayed files from deletion, preserve concurrent/conflicting edits, and exercise a durable copy transaction interrupted both before and after publication. A built-app smoke mode proves launch against an explicitly validated disposable temporary directory without opening real preferences or notes. GitHub Actions and the local scripts are intended to run the complete app build, smoke test, both XCTest targets, and all characterization executables in Debug and Release; the workflow now watches the repository's `master` branch. The recorded clean-build ceiling is 875 repository warnings per configuration, but current clean Debug and Release builds report 877 and stop the composed safety net before downstream tests. Restore the warning count to the ceiling or lower before treating Phase 1 as green. Representative full NV/nvAlt database archives remain Phase 2 work, and ongoing coordinated multi-client iCloud behavior remains Phase 4 work.
...
**Status as of August 2026 — substantial implementation, exit criterion not met:** `Shared/SpiralCore` provides the platform-neutral `Note` domain with permanent UUID identity, deterministic metadata-free TXT/RTF/HTML codecs, versioned independent reconciliation records, conflict and index-event values, an actor-isolated local `NoteStore`, a disposable rebuildable index, mixed separate-file import, and a copy-only legacy migration transaction with byte-verified retained backup and rollback. Valid source extensions such as `.md`, `.taskpaper`, and `.htm` are preserved through ordinary updates, renames, and moves; only an explicit format change selects the new format's preferred extension. The Mac's archive, encryption, passphrase, and WAL implementation is quarantined behind `LegacyCompatibility/NVLegacyCompatibility.h`, and the retained Phase 1 tests cover lower-level cipher, KDF, archive-metadata, encoding, and intact/torn/wrong-key WAL behavior. Encrypted imports require explicit confirmation that the destination is ordinary plaintext and preserve both the encrypted source and its Keychain item.

The remaining Phase 2 work is material. The package's NV/nvAlt fixtures are small decoded-value JSON snapshots rather than representative full archives, and the production Objective-C compatibility reader is not connected end to end to `LegacyMigrationService`. Add sanitized unencrypted, encrypted, alternate-KDF/cipher, intact/interrupted-WAL, wrong-passphrase, and damaged historical archives; open or recover them through the production compatibility path; verify note and metadata values; migrate through the shared store; and prove rollback and source immutability. The codec retains unchanged externally produced RTF/HTML bytes and now refuses a text-only mutation of loaded rich content; store tests prove the original file and in-memory note remain unchanged on refusal. A format-preserving model, editor adapter, serializer, and edit fixtures are still required before rich bodies become editable. `Scripts/ci/run-phase2.sh` composes the package and application suites, but it currently stops at the Phase 1 warning gate.
...
Existing RTF and HTML bodies are now read-only in both shared editor adapters, with an explanatory UI message; model and codec guards reject text-only rich-file mutations while title, tag, pin, and deletion operations remain available. This prevents the previously possible flattening regression but does not provide rich-content editing. The UI-test sources describe compact and split navigation, keyboard commands, accessibility labels, accessibility-size Dynamic Type, rotation/resizing, restoration, and an identical 121-note disposable fixture on iPhone, iPad, and Mac. `Scripts/ci/run-phase3.sh`, however, only performs `build-for-testing` for the mobile and Mac UI targets; it does not execute either suite, and the current warning-gate failure prevents those build checks from being reached. Add explicit iPhone, iPad, and Mac test destinations and execute the UI suites before declaring Phase 3 complete.
...
1. Restore the Phase 1 safety net to green by reducing the current Debug and Release warning counts from 877 to the recorded ceiling of 875 or lower. Keep the corrected `master` push trigger and verify a complete composed CI run reaches every downstream test.
2. Complete Phase 2 legacy coverage with sanitized representative Notational Velocity and nvAlt archives for unencrypted, encrypted, alternate-KDF/cipher, intact and interrupted WAL, wrong-passphrase, damaged, encoding, filename, and historical-metadata cases. Connect the production Objective-C compatibility reader to `LegacyMigrationService` and verify note-for-note and metadata-for-metadata conversion, rollback, source immutability, and Keychain preservation.
3. Replace the shared plain-string RTF/HTML representation with a format-preserving model, editor adapters, and serializers. The interim read-only UI and codec/store refusal are implemented; retain those guards until the replacement passes fixtures that insert, delete, and format text while proving surrounding RTF attributes and HTML structure survive.
