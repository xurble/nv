...
This is a transitional bridge into the Phase 4 production store. A database-only or local legacy collection receives one guarded Mac launch to make the verified copy and generate canonical files. On the next launch, Mac verifies and retains a local backup of `Notes & Settings` and `Interim Note-Changes`, removes only those backed-up legacy artifacts from public iCloud `Documents`, and opens `CloudNoteStore`; canonical notes are never removed by that retirement transaction. The iPhone/iPad app refuses a public directory containing legacy artifacts and directs the user to finish the handoff on Mac. After handoff, all three production targets use the same coordinated clean files, private synchronized reconciliation records, and local rebuildable indexes.
...
**Progress as of August 2026 — production boundary selected; live verification is the next migration goal:** `FoundationCloudDocumentAdapter` provides the OS 26-compatible boundary for ubiquity-container discovery, coordinated reads/writes/moves/deletes, unavailable-item download requests, and unresolved `NSFileVersion` conflicts on macOS, iOS, and iPadOS. `CloudDocumentPresenter`, `CloudMetadataMonitor`, and `CloudCollectionObserver` turn file-presentation, metadata, launch, and foreground changes into full reconciliation scans without exposing those APIs to feature code. `CloudCollectionReconciler` matches current path, recent paths, then hashes; refuses ambiguous or duplicate UUID assignments; distinguishes unavailable files from confirmed deletion; merges validated non-overlapping UTF-8 line edits; and preserves visible conflict copies for same-line, invalid-encoding, RTF, and HTML divergence.

The copy-only `CloudCollectionMigrationService` publishes canonical clean files and private reconciliation records through those adapters. Its atomically rewritten per-item journal resumes after interruption, retains a byte-verified source backup, verifies successful publication, and rolls back only unchanged transaction-owned bytes. Deterministic two-device different-note and three-device same-note fault tests cover delayed reconciliation records, non-overlapping convergence, same-line conflict preservation, interrupted publication, resume, rollback, and refusal to erase a post-publication external edit. The existing live legacy-controller merge now uses a verified transaction object: identical UUID matches are skipped, divergent matches receive collision-safe “Merged Copy” titles, successful publication removes its backup, failed publication stages and verifies restoration, and failed restoration retains the backup. Characterization tests cover all of those decisions and injected failures.

`CloudNoteStore` now implements the production `NoteStore` contract over coordinated public documents and private synchronized reconciliation records, with only its disposable index kept in local Application Support. The iOS target carries the same iCloud entitlements and container identifier as Mac. Production iPhone/iPad launches use this store, full-scan on foreground and cloud signals, and refuse legacy or unrelated public data. Mac selects the same store when its current collection is already canonical; a verified retained-backup transaction retires legacy database/WAL artifacts only after canonical files exist. A two-client regression proves Mac creation, mobile edit, stable UUID retention, and Mac reload through one adapter-backed collection. Register and verify the intended container, then run account-change, eviction, storage-exhaustion, unrelated-editor, and physical two-/three-device tests before declaring the phase exit criterion met.
...
6. **Next migration goal:** register the shared container and execute the Phase 4 production matrix on physical Mac, iPhone, and iPad devices: first legacy handoff, simultaneous edits, offline convergence, account changes, eviction, storage exhaustion, external editor writes, conflict recovery, and interrupted networks. Fix any failures before release.
7. Implement the Phase 5 OS 26 UUID-backed `NoteEntity`, protected Core Spotlight index, deep-link/open behavior, privacy controls, incremental updates, full rebuild, and filesystem-duplicate characterization.
