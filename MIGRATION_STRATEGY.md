...
The shared SwiftUI feature still loads full `Note` values and performs its visible filtering in memory, so the summary/body split is not yet complete at the UI boundary. Bounded hydration, file protection verification, account-change lifecycle handling, protected Core Spotlight, `NoteEntity`, deep links, user-facing privacy controls, and measured 1,000-/10,000-note Mac/iPhone/iPad performance coverage remain outstanding.
...
6. **Current migration goal — complete Phase 5:** route the shared feature list/search UI through the implemented SQLite summary/FTS boundary, finish on-demand body loading and bounded hydration, then ship the UUID-backed protected Core Spotlight projection, alias-aware deep links, privacy controls, rebuild/file-protection/account-change handling, and measured 1,000-/10,000-note Mac/iPhone/iPad coverage.
