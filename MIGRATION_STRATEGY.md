...
- handles delayed and independently ordered document/metadata delivery, offload, offline edits, coordinated writes, external changes, and file conflicts explicitly;
...
- provides effective local indexed search on Mac, iPhone, and iPad even when previously indexed canonical bodies are offloaded, while honestly reporting never-indexed coverage;
...
The project contains approximately 40,000 lines of Objective-C and C. Its largest and most interconnected classes include `NoteObject`, `AppController`, `NotationController`, `LinkingEditor`, and `NotesTableView`. Phase 1 provides separate XCTest unit and disposable-directory integration targets plus focused characterization executables for critical legacy seams, and its complete Debug and Release safety-net runs are green below the recorded warning ceiling. Phase 2 has met its exit criterion: the shared storage boundary now exercises representative full Notational Velocity and nvAlt archives through the production compatibility reader and shared migration service, and its lossless token document permits format-preserving RTF/HTML editing. Phase 3 has also met its exit criterion: the universal mobile target and shared SwiftUI feature operate on the same disposable collection in executed iPhone, iPad, and Mac UI suites. Phase 4 supplies an OS 26 iCloud document-store baseline and deterministic fault harness, but design review found that asynchronous sidecar arrival, tombstone ordering, offload, record conflicts, scan amplification, and collection-wide in-memory search need further architecture before physical-device release approval. Phase 5—durable local catalog, indexed in-app search, and protected Core Spotlight integration—is therefore the next migration phase. Phase 6 then hardens reconciliation and completes the registered-container and physical-device release gate using that catalog and availability model.
...
9. **Keep cloud data and local indexes separate.** Rebuildable search indexes, thumbnails, caches, and local database accelerators must not be synchronized as user documents. Keep them durable per client so offload does not silently erase known summaries or previously indexed search coverage.
...
- **ReconciliationStore (shared Swift package):** private, per-note reconciliation records stored outside the public `Documents` directory of Spiral's iCloud ubiquity container. Local-only collections use the equivalent private Application Support location and migrate their records when the collection explicitly moves to iCloud. A record maps a UUID to its origin, current and recent relative paths, in-progress path transition, raw-content hashes, bounded merge ancestry, independently versioned app metadata, tombstone generation, deletion state, and optional repaired-identity alias. Use one independently replaceable record per note rather than one global synchronized database. Enumerate and merge record `NSFileVersion` conflicts. An unavailable record places an observed document in `awaitingMetadata`; it never causes immediate permanent identity creation.
- **NoteStore (shared Swift package):** an actor-isolated storage contract for paged `NoteSummary` enumeration, indexed search, on-demand `NoteBody` loading, explicit downloads, create/update/delete, coordinated saves, change observation, conflict and deletion repair, migration, and index events. Summaries expose body/metadata availability and search freshness without leaking platform file APIs. It must not expose `AppKit`, `UIKit`, SwiftUI scene state, or Objective-C model objects.
- **Document adapters:** because iOS and iPadOS 26 are supported, implement the Phase 4 document layer behind an OS 26-compatible adapter without exposing platform availability types to feature code. Phase 10 may replace or extend that adapter with the OS 27 SwiftUI `Document`/`ReadableDocument`/`WritableDocument` APIs, which provide async URL access, snapshots, progress, and a framework-provided file coordinator.
- **Local catalog and indexes:** an account-scoped SQLite catalog in Application Support stores verified summaries, availability, identity aliases, pending operations, and an FTS index of permitted last-verified text. A named protected Core Spotlight index receives an incremental projection of user-approved notes. Both are local, rebuildable, never authoritative, and remain useful when canonical files are offloaded. If SwiftData-to-CloudKit synchronization is later used for app-only metadata, keep its store local and treat file delivery and CloudKit delivery as independent, asynchronously ordered channels.
...
Objective-C and Swift can coexist while the boundary is extracted. C should remain only for small, stable legacy compatibility algorithms. New boundaries use typed async APIs and errors rather than notifications, dynamic selectors, or shared controller state. [`ARCHITECTURE.md`](ARCHITECTURE.md) defines the complete target pairing, indexing, offload, mutation, and reconciliation behavior.
...
- reconciliation records carry origin, independently versioned metadata fields, transition state, tombstone generation, and repaired-identity aliases;
- tombstones reserve their UUID, current path, recent paths, and deleted hash while document removal is in flight, preventing an offline or late client from rediscovering the bytes as a new note;
...
- both the document and reconciliation directories are observed; signals are coalesced into an incremental dirty set, with read-only integrity scans at launch and foregrounding because uncoordinated external writes do not reliably generate file-presenter notifications;
- unchanged scans cause no cloud writes, metadata-only mutations do not rewrite document bytes, and ordinary saves perform work proportional to the changed note rather than the collection;
- a locally absent iCloud file is not treated as deleted until tombstone state or explicit confirmation distinguishes deletion from eviction or delayed download;
- an unmatched document enters local provisional discovery and can be superseded by a late authoritative record; mere record absence never immediately creates a permanent identity; and
...
Use a title-derived filename with collision handling. App-coordinated renames retain the UUID. Reconciliation first claims every exact current path across the collection, then transition/recent paths, then unique content ancestry; do not greedily hash-match one enumerated file at a time. This ordering ensures a lexicographically earlier copy cannot steal the original path's UUID. A new unmatched file begins as a local provisional discovery. After unavailable records have been requested and metadata observation reaches quiescence, clients may publish the same deterministic `externalDiscovery` identity for the same normalized path and bytes. If an older app-created or migration record arrives later and uniquely matches path/hash ancestry, it wins; the discovered identity becomes a retained `supersededBy` alias so existing links continue to resolve. A rename combined with a content edit outside Spiral can be indistinguishable from delete-plus-create or copy-plus-edit because the file carries no stable identifier. When identity or a merge is ambiguous, preserve both versions and ask the user rather than silently assigning history or overwriting content.

Every create, move, and delete uses a recoverable local operation journal plus synchronized transition state in the affected record. Create publishes an expected-document record before the document; move publishes old/new path and expected-hash transition data; delete publishes a tombstone before removing the document. Either side may still arrive first on another client, but the receiving client exposes `awaitingBody`, `awaitingMetadata`, or `deletionInFlight` instead of inventing identity or resurrecting content.

The reconciliation history is not an unlimited second copy of the collection. Retain only the revision ancestry and bounded common-base content required for conflict recovery, avoid rewriting it when bytes are unchanged, define a size-and-age deletion policy, and ensure deleting a note eventually makes its private history eligible for deletion. Keep the account-scoped SQLite catalog and FTS store in local Application Support, never iCloud Drive. It retains verified summaries, availability, aliases, and permitted last-indexed text across document offload. A protected Core Spotlight index contains only the user-approved projection and uses the permanent UUID for navigation.
...
This is a transitional bridge into the Phase 4 iCloud-store baseline. A database-only or local legacy collection receives one guarded Mac launch to make the verified copy and generate canonical files. On the next launch, Mac verifies and retains a local backup of `Notes & Settings` and `Interim Note-Changes`, removes only those backed-up legacy artifacts from public iCloud `Documents`, and opens `CloudNoteStore`; canonical notes are never removed by that retirement transaction. The iPhone/iPad app refuses a public directory containing legacy artifacts and directs the user to finish the handoff on Mac. After handoff, all three targets use the same coordinated clean files and private synchronized reconciliation records. Phase 5 replaces the prototype index, and Phase 6 must satisfy the multi-client release gate before this path is production-enabled.
...
## OS 26 Local and Spotlight Search

Core Spotlight and `IndexedEntity` do not depend on the OS 27 Notes App Schema. Because OS 26 is the product minimum and collection-wide in-memory search cannot provide honest offload behavior or large-library performance, the durable local catalog and first-class Spotlight work is the next migration phase. Do not wait for the deferred OS 27 Siri work or for registered-container release approval.

Implement the following in shared system-integration sources used by the Mac and iOS applications:

1. **Summary/body split.** Replace collection-wide `[Note]` loading with paged `NoteSummary` enumeration and on-demand `NoteBody` loading. Summaries expose body/metadata availability, last verified hash, search freshness, and conflict or repair state.
2. **Durable local catalog.** Add an account-scoped SQLite catalog in Application Support for verified UUID/path/hash mappings, summaries, availability, aliases, pending operations, and FTS content. It is rebuildable but is not discarded merely because canonical files are temporarily offloaded.
3. **Effective in-app search.** Route Mac, iPhone, and iPad search through `NoteStore.search`, with tokenized multi-term matching, relevance, snippets, pagination, and explicit incomplete/stale indexing state. Stop filtering and sorting every decoded body on the main actor.
4. **Stable generic entity.** Define `NoteEntity` as an `IndexedEntity` without adopting the OS 27 Notes schema. Use only an authoritative or repaired permanent note UUID as its identifier; never donate a purely local provisional identity, title, filename, path, file-resource identifier, or legacy volume metadata as Spotlight identity.
5. **Entity resolution.** Implement an async entity query that resolves UUIDs and retained identity aliases through `NoteStore`. It must work without an AppKit controller, foreground window, or live legacy object graph.
6. **Protected system index.** Donate eligible note entities to a named, protected Core Spotlight index using title, textual content, tags, and created/modified dates. Associate the canonical file URL where appropriate. Update or delete entries after saves, renames, moves, deletes, identity repair, conflict resolution, import, migration, privacy changes, and account changes.
7. **Opening and search actions.** Add an `OpenIntent` and UUID-based deep links so selecting a result opens the exact note after rename, move, offload, or repaired provisional identity. Expose focused Open Note and Search Notes actions; keep mutations out of this milestone.
8. **Hydration and recovery.** Treat both local indexes as non-authoritative but durable acceleration. Preserve the last verified permitted body after canonical offload. Use a bounded, resumable background indexer for bodies never downloaded on this client, subject to network, power, storage, and privacy policy. Collection opening must not synchronously request every file. Support deterministic rebuild and system-requested reindexing.
9. **Privacy and native-file indexing.** Make Spotlight donation an explicit collection-level setting with per-note exclusion, omit unavailable legacy-encrypted content, and never donate transient plaintext produced during migration. Because canonical `.txt`, `.rtf`, and `.html` files in Finder-visible or iCloud Drive locations may also be indexed independently by macOS, document that disabling Spiral's donation does not by itself guarantee removal of filesystem-generated Spotlight results. Characterize duplicate-result and exclusion behavior before promising complete Spotlight privacy.
10. **Testing.** Add disposable-collection coverage for initial and incremental indexing, deletion, full rebuild, UUID and alias deep links, rename/move, locked legacy collections, privacy changes, account partitioning, unavailable and never-indexed iCloud files, index completeness, conflicts, and possible duplication with filesystem-indexed canonical files. Measure cold/warm query latency and memory with 1,000- and 10,000-note collections on Mac, iPhone, and iPad.

This OS 26 work establishes the availability-aware store boundary, durable local search catalog, Spotlight entity, privacy policy, and navigation contract needed by Phase 6 reconciliation hardening. The OS 27 work extends those same identities rather than replacing or reindexing them under a new identifier scheme.
...
- simulated iCloud Drive states including unavailable files, delayed downloads, concurrent edits, conflicts, and interrupted migration;
- document-first, record-first, tombstone-first, deletion-first, record-offloaded, both-offloaded, late-authoritative-record, and account-change sequences;
- concurrent independent metadata edits, reconciliation-record `NSFileVersion` conflicts, external-copy enumeration order, and provisional-identity repair;
- 1,000- and 10,000-note search, launch, save, bulk-hydration, scan-write-amplification, memory, and energy measurements; and
...
**Status as of August 2026 — exit criterion met:** `Shared/SpiralFeature` supplies the common SwiftUI collection shell and `SpiralFeatureModel` over the Phase 2 `NoteStore`, including adaptive navigation/search, list and editor hosting, create/rename/tag/pin/delete, settings, and explicit loading/empty/download/error/conflict states. The universal `SpiralMobile` target supports iPhone and iPad and links only `SpiralCore` and `SpiralFeature`; production now opens the same coordinated iCloud collection as Mac, while UI tests retain their sentinel-validated local Application Support fixtures. iOS editing uses a Dynamic Type-aware `UITextView` adapter with selection preservation. The Mac shell reuses the same feature while injecting the established `LinkingEditor` through `NVSettingsBridge`; its test mode never opens a legacy user collection. Model tests exercise the complete plain-text mutation workflow and a 1,000-note collection.

The current search implementation remains a Phase 3 prototype: it loads every decoded note, filters bodies on the main actor, and has only functional—not performance—coverage at 1,000 tiny in-memory notes. Phase 5 replaces that path with paged summaries, on-demand bodies, durable FTS, protected Core Spotlight projection, and measured Mac/iPhone/iPad performance.

RTF and HTML bodies are editable in both shared editor adapters through the format-preserving Phase 2 model. The model and codec retain a refusal guard against direct plain-string mutation, and tests cover the rich editing path separately. The UI suites execute on explicit iPhone 17 Pro, iPad Pro 13-inch, and native Mac destinations and cover compact and split navigation, keyboard commands, accessibility labels, accessibility-size Dynamic Type, rotation/full-screen resizing, restoration, and an identical 121-note disposable fixture. The mobile restoration workflow waits for the durable store update before relaunching. Mac test mode accepts only a randomized, sentinel-authenticated XCTest temporary root and refuses unverified paths, so it cannot open a real notes collection. `Scripts/ci/run-phase3.sh` now runs, rather than merely compiles, all three UI legs; the destinations can be overridden for another installed OS 26 simulator runtime.

### Phase 4: Establish the iCloud Document-Store Baseline
...
- Implement the iPhone/iPad document layer through an OS 26-compatible adapter; keep availability-specific implementation out of `NoteStore` and defer adoption of OS 27-only `Document` APIs to Phase 10.
...
**Baseline exit criterion:** The repository has a shared OS 26 iCloud adapter, per-note content reconciliation, deterministic fault adapters, and durable migration publication suitable for the Phase 5 index boundary and Phase 6 hardening work. This baseline is not permission to ship the shared store.

**Progress as of August 2026 — baseline implemented, production release gate not met:** `FoundationCloudDocumentAdapter` provides the OS 26-compatible boundary for ubiquity-container discovery, coordinated reads/writes/moves/deletes, unavailable-item download requests, and unresolved document `NSFileVersion` conflicts on macOS, iOS, and iPadOS. `CloudDocumentPresenter`, `CloudMetadataMonitor`, and `CloudCollectionObserver` currently turn public-document presentation, metadata, launch, and mobile foreground changes into full reconciliation scans. `CloudCollectionReconciler` matches current path, recent paths, then hashes; refuses some ambiguous or duplicate UUID assignments; distinguishes unavailable documents from confirmed deletion; merges validated non-overlapping UTF-8 line edits; and preserves visible conflict copies for same-line, invalid-encoding, RTF, and HTML divergence.

The copy-only `CloudCollectionMigrationService` publishes canonical clean files and private reconciliation records through those adapters. Its atomically rewritten per-item journal resumes after interruption, retains a byte-verified source backup, verifies successful publication, and rolls back only unchanged transaction-owned bytes. Existing two-device different-note and three-device same-note tests cover stale record replicas, non-overlapping convergence, same-line conflict preservation, interrupted publication, resume, rollback, and refusal to erase a post-publication external edit. They do not yet model every independent document/record/tombstone delivery order, record `NSFileVersion` conflict, real offload transition, or separate document replicas. The existing live legacy-controller merge now uses a verified transaction object: identical UUID matches are skipped, divergent matches receive collision-safe “Merged Copy” titles, successful publication removes its backup, failed publication stages and verifies restoration, and failed restoration retains the backup.

`CloudNoteStore` implements the baseline `NoteStore` over coordinated public documents and private synchronized records. Design review identified release-blocking gaps: a document received before its random-UUID record can be rediscovered under a second UUID; a tombstone received before document removal can permit resurrection; unavailable records are skipped while available documents are classified; record-only arrivals are not observed directly; record conflicts and independent metadata fields are not merged; warm and cold offload produce inconsistent visibility; initial all-offloaded loads can fail before observers start; scans reread bodies and rewrite unchanged records; and in-memory search is incomplete when bodies are not hydrated. Phase 5 supplies the durable availability/search catalog needed to represent those states. Phase 6 corrects the reconciliation protocol and completes live verification before the store is release-enabled.

### Phase 5: Ship the Local Catalog and OS 26 Spotlight Search

- Make the OS 26 local-catalog and Spotlight plan above the immediate next migration outcome; do not wait for physical iCloud validation or the OS 27 Notes schema.
- Split `Note` loading into paged summaries and on-demand bodies with explicit body/metadata availability and search freshness.
- Add the account-scoped SQLite catalog and FTS schema, including verified UUID/path/hash mappings, summaries, cached permitted text, aliases, provisional discoveries, and operation state.
- Replace in-memory substring filtering with `NoteStore.search`, ranked snippets, pagination, index completeness, and download/freshness presentation shared by Mac, iPhone, and iPad.
- Add the generic UUID-backed `NoteEntity`, named protected Core Spotlight projection, alias-aware entity resolution, deep links, Open Note, and Search Notes.
- Keep SQLite and Spotlight incrementally current through `NoteStore` index events. Do not rebuild either index after every mutation or discard verified rows because a body is offloaded.
- Add the bounded background hydration and rebuild policy. Never synchronously download every unavailable note while opening the collection.
- Add an opt-in collection policy with per-note exclusion, omit locked or unmigrated legacy-encrypted content, purge results on account changes, and characterize duplicate/exclusion behavior for Finder-visible canonical files.
- Do not adopt the OS 27 Notes schema, `SyncableEntity`, Siri create/update intents, or OS 27-only document/reindexing APIs in this phase.

**Exit criterion:** On Mac, iPhone, and iPad, a disposable collection opens from summaries without decoding every body; previously indexed offloaded notes remain honestly searchable; never-indexed bodies report incomplete coverage while bounded hydration progresses; eligible notes appear in protected Spotlight search; UUID and alias links survive rename/move and identity repair; update/delete/privacy/account-change/rebuild behavior is covered; and measured 1,000-/10,000-note query, launch, memory, and save results meet documented budgets.

**Progress as of August 2026 — foundation implemented, phase exit criterion not yet met:** `SpiralCore` now has a versioned, account-and-collection-scoped SQLite catalog with FTS5, paged `NoteSummary` enumeration, ranked/prefix/phrase search, snippets, explicit body/metadata availability, search freshness, completeness counts, UUID aliases, provisional-discovery rows, and pending-operation summaries. `LocalNoteStore` and `CloudNoteStore` incrementally update affected rows and expose `NoteStore.search`; a previously indexed body remains searchable as stale after offload, while a never-indexed body reports incomplete coverage. Private notes are excluded by default, their cached body is purged with SQLite secure deletion and a truncated WAL, the iOS catalog directory uses complete-until-first-authentication file protection, and a different iCloud account token cannot reopen the catalog. The catalog is local and rebuildable; canonical note files and reconciliation records remain authoritative.

The shared SwiftUI feature now opens the collection from a bounded first page of `NoteSummary` values, paginates list and search results, debounces queries through `NoteStore.search`, preserves catalog relevance and snippets, and presents completeness plus cloud/freshness state on Mac, iPhone, and iPad. It calls `note(id:)` only for the selected row. An unavailable or stale indexed body gets a non-editable detail state, so title, tag, pin, and body mutations cannot save a cached projection over an unavailable canonical revision. A 1,000-note feature test proves opening and indexed search do not call `allNotes()`, load only the selected body, and can return results outside the loaded summary page.

Body hydration is now policy-bound instead of an implicit side effect of reconciliation. Opening an offloaded collection requests no canonical bodies during `CloudCollectionReconciler`; selecting a row requests only that body, while the shared feature schedules a resumable four-item search-index batch. `CloudNoteStore` enforces an eight-item hard ceiling for callers, counts existing `downloadPending` rows against the limit, prioritizes eligible `neverIndexed` notes, and retains stale indexed text without redownloading it. Observer-driven reloads index completed bodies and schedule the next bounded batch. Deterministic 10-note coverage proves that a three-item policy leaves exactly three pending and seven queued, issues no additional requests while those three remain pending, and resumes with the next three after completion.

The protected Core Spotlight projection, `NoteEntity`, and alias-aware deep links are now the next Phase 5 implementation step. File protection verification, account-change lifecycle handling, user-facing privacy controls, and measured 1,000-/10,000-note Mac/iPhone/iPad performance coverage also remain outstanding.

### Phase 6: Harden Multi-Client Reconciliation and Complete the iCloud Release Gate

- Evolve the reconciliation schema with record origin, per-field revisions, per-tag add/remove revisions, content ancestry isolated from metadata edits, path transition state, tombstone generation, deletion completion, and `supersededBy` aliases.
- Implement the explicit pairing states from `ARCHITECTURE.md`: `awaitingMetadata`, `awaitingBody`, `provisionalDiscovery`, `deletionInFlight`, `missingPendingConfirmation`, and repair conflict.
- Never create a permanent UUID merely because a record is absent or unavailable. Request placeholders, consult the verified local catalog, wait for metadata-query quiescence, and publish only reversible deterministic `externalDiscovery` records. Allow a uniquely matching late app-created or migration record to supersede discovery while retaining an alias.
- Replace greedy enumeration matching with collection-wide passes: exact current paths first, transitions/recent paths second, then unique content ancestry. Add copy-order characterization.
- Add recoverable journals and synchronized transition state for ordinary create, move, and delete operations. Publish tombstones before document removal so either replication order is non-resurrecting.
- Observe both `Documents` and `Data/Reconciliation`. Start observation before initial hydration. Coalesce signals into one active task plus a dirty set; reconcile incrementally and keep full startup/foreground scans read-only for unchanged state.
- Stop hashing and decoding every body during ordinary enumeration. Do not rewrite unchanged records, do not rewrite canonical bytes for metadata-only changes, and make one-note saves proportional to that note.
- Enumerate and resolve reconciliation-record `NSFileVersion` conflicts. Merge metadata by field and tags per normalized tag; never replace newer content ancestry during a metadata-only merge.
- Add user workflows for choose-local, choose-external, manual merge, keep-both, mark-resolved, external deletion confirmation, possible same-path replacement, duplicate authoritative identity, and interrupted-transition recovery.
- Define bounded merge-base, tombstone, transition, and identity-alias retention and garbage collection.
- Quarantine and report unsupported visible items without preventing valid notes from opening or modifying those items.
- Observe iCloud identity changes; cancel active work, clear memory, close the account-scoped catalog, deactivate the old Spotlight projection, and resolve the new container before displaying notes.
- Add deterministic fault coverage for every document/record/tombstone arrival order, record offload, both-side offload, late authoritative repair, concurrent metadata, separate public replicas, storage exhaustion, termination, and network interruption.
- Register the shared container and run the physical Mac/iPhone/iPad matrix only after those tests pass. Measure unchanged-scan writes, bulk hydration, launch, search, save, memory, and energy with 1,000 and 10,000 mixed-format notes.

**Exit criterion:** Multiple physical clients can list and search honestly under offload, edit different notes offline and converge, preserve every ambiguous same-note version, merge independent metadata without lost updates, avoid duplicate identity and resurrection in every delivery order, recover interrupted operations, change accounts without cache disclosure, and meet performance budgets without unchanged cloud-write amplification.

**Progress as of August 2026:** The Phase 4 baseline and deterministic adapters exist, but the pairing state machine, reversible discovery, record-version merge, incremental observer, operation journal for ordinary mutations, repair UI, retention policy, account isolation, and registered-container physical-device matrix remain outstanding.

### Phase 7: Make the Products Reproducible and Distributable
...
**Progress as of August 2026:** The application builds with the current Xcode and macOS SDK, and both app targets declare the shared `iCloud.farm.poplar.spiral` Documents container. Fresh installations use this container when it is available. A first run that detects Notational Velocity or nvAlt preferences offers the guarded, folder-selected, copy-only import described above; the manual switch workflow offers Copy or Keep and no longer offers Move. After the Mac's guarded legacy handoff, all three platforms select the Phase 4 coordinated per-note baseline and keep reconciliation records outside public `Documents`. Container registration and signed physical-device verification with the intended Apple Developer team are Phase 6 release gates.
...
### Phase 8: Replace Obsolete Dependencies and Deprecated Platform APIs
...
### Phase 9: Quarantine and Shrink the Legacy Mac Core
...
### Phase 10: Add OS 27 Document APIs, Notes Schema, App Intents, and Siri
...
4. **Completed August 2026:** Executed the Phase 3 suites on an iPhone 17 Pro simulator, iPad Pro 13-inch simulator, and native arm64 Mac, all on OS 26.5. The tests cover compact/split navigation, keyboard workflows, accessibility, Dynamic Type, rotation/full-screen resizing, restoration, and the same disposable 121-note collection. The Phase 3 gate now runs each UI suite on an explicit overridable destination, and the Mac fixture shell requires a randomized sentinel-authenticated XCTest path before it opens any files.
5. **Completed August 2026:** Characterized and hardened the live legacy merge for identical/divergent UUIDs, case-insensitive merged-title collisions, verified backup cleanup, publication, rollback, and injected restore failures. Added the OS 26 Foundation iCloud adapter, presenter/metadata signals, deterministic identity and format-aware reconciliation, visible conflict copies, a per-item durable copy-only migration journal with guarded rollback, and two-/three-device fault tests. The shared production collection remains intentionally disabled pending registered-container and physical-device verification.
6. **Current migration goal — complete Phase 5:** ship the UUID-backed protected Core Spotlight projection and alias-aware deep links on top of the implemented summary/search/on-demand-body/bounded-hydration boundary, then add privacy controls, rebuild/file-protection/account-change handling, and measured 1,000-/10,000-note Mac/iPhone/iPad coverage.
7. Complete Phase 6 reconciliation hardening: reversible external discovery, late-authoritative repair, tombstone-first deletion, operation journals and transition state, collection-wide identity matching, record field/version merge, dual-directory observation, incremental dirty reconciliation, repair UI, retention, account isolation, and the registered-container physical matrix under offload, offline edits, storage exhaustion, external editors, and interrupted networks.
8. Complete Phase 7 build hygiene: remove macOS 10.5/10.9 and legacy SDK overrides; align all app targets, test targets, and package manifests to macOS/iOS/iPadOS 26 or later; make the intended Debug product universal; remove tracked `xcuserdata`; expand `.xcconfig` ownership; and verify unsigned builds, signed archives, hardened runtime, notarization, TestFlight/App Store signing, and clean-machine launch.
9. Complete Phase 8 removals one subsystem at a time: delete the unused AutoHyperlinks files; remove inactive Simplenote/sync/JSON/hashcash scaffolding after compatibility verification; and replace Carbon/`FSRef`, Launch Services, synchronous panels, unsafe archives, legacy WebKit, file notifications, RBSplitView, PTHotKeys, and ODBEditor with tested supported boundaries.
10. Continue Phase 9 with typed protocols and direct calls, isolated ARC leaf conversions, broader nullability/generics, static analysis, and memory diagnostics before shrinking the large legacy controllers. After Phases 5–9 establish stable OS 26 behavior, implement Phase 10 OS 27 document APIs, Notes schema, `SyncableEntity`, Siri create/update actions, onscreen awareness, and App Intents Testing without changing UUIDs or dropping OS 26 compatibility.
...
- stable note IDs survive app-coordinated rename, move, device changes, offload, either document/record arrival order, tombstone ordering, late authoritative repair, and conflict recovery; externally ambiguous replacement, rename-plus-edit, and copy-versus-move cases preserve both notes instead of claiming false identity certainty;
- record conflicts merge independent metadata fields and tags without overwriting newer content ancestry, and tombstones prevent resurrection whichever side arrives first;
- warm and cold clients expose honest summary, body, metadata, download, and search-freshness state; previously indexed offloaded notes remain searchable according to policy and never-indexed coverage is visible;
- ordinary saves and incremental reconciliation perform work proportional to changed notes, unchanged scans produce no iCloud writes, and documented 1,000-/10,000-note performance budgets pass on Mac, iPhone, and iPad;
- iCloud account changes isolate memory, local catalogs, pending operations, and Spotlight projections so a new account cannot query the previous account's cached notes;
- interactive conflict, deletion-confirmation, possible-replacement, identity-repair, and interrupted-operation workflows are available before the shared store is release-enabled;
