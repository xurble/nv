...
**Progress as of August 2026 — implementation boundary complete, production enablement withheld:** `FoundationCloudDocumentAdapter` provides the OS 26-compatible boundary for ubiquity-container discovery, coordinated reads/writes/moves/deletes, unavailable-item download requests, and unresolved `NSFileVersion` conflicts on macOS, iOS, and iPadOS. `CloudDocumentPresenter` and `CloudMetadataMonitor` turn `NSFilePresenter` and `NSMetadataQuery` changes into full-scan signals without exposing those APIs to `NoteStore`. `CloudCollectionReconciler` matches current path, recent paths, then hashes; refuses ambiguous or duplicate UUID assignments; distinguishes unavailable files from confirmed deletion; merges validated non-overlapping UTF-8 line edits; and preserves visible conflict copies for same-line, invalid-encoding, RTF, and HTML divergence.
...
The current Mac compatibility controller and the mobile app are not switched to the shared iCloud collection. Register and verify the intended container, then run account-change, eviction, storage-exhaustion, unrelated-editor, and physical two-/three-device tests before selecting this boundary in production and declaring the phase exit criterion met.
